...
# ---------------------------------------------------------------------------
# Trailing Text Trimmer
# ---------------------------------------------------------------------------

def strip_trailing_text_after_mermaid(text: str) -> str:
    """
    If a Mermaid diagram is present, ensure absolutely no text appears after it.
    This fixes issues where the LLM appends trailing remarks after the diagram.
    """
    matches = list(re.finditer(r"```mermaid.*?```", text, flags=re.DOTALL | re.IGNORECASE))
    if not matches:
        return text
...
def format_agent_output(text: str) -> str:
    ...
    Processing order:
        ...
        5. Trim trailing text after Mermaid diagrams
    ...
    text = strip_trailing_text_after_mermaid(text)
    return text
